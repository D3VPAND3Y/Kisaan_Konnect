...
// Correct the path to the models
const cropModelPath = path.join(__dirname, '../frontend/src/models/Crop_prediction.onnx');
const fertilizerModelPath = path.join(__dirname, '../frontend/src/models/random_forest_classifier.onnx');
const cropDiseaseModelPath = path.join(__dirname, '../frontend/src/models/model.onnx'); // Example path for the crop disease model

console.log("Loading Crop model from: ", cropModelPath);
console.log("Loading Fertilizer model from: ", fertilizerModelPath);
console.log("Loading Crop Disease model from: ", cropDiseaseModelPath);

// Load ONNX Models
let cropSession;
let fertilizerSession;
let cropDiseaseSession;

// Load Crop Prediction Model
onnx.InferenceSession.create(cropModelPath)
  .then((s) => {
    cropSession = s;
    console.log('Crop Model loaded successfully');
  })
  .catch((err) => {
    console.error('Failed to load Crop model:', err);
  });

// Load Fertilizer Prediction Model
onnx.InferenceSession.create(fertilizerModelPath)
  .then((s) => {
    fertilizerSession = s;
    console.log('Fertilizer Model loaded successfully');
  })
  .catch((err) => {
    console.error('Failed to load Fertilizer model:', err);
  });

// Load Crop Disease Prediction Model
onnx.InferenceSession.create(cropDiseaseModelPath)
  .then((s) => {
